...
async def create_todo(
    ...
    reminder_time: Optional[Union[int, str]] = None,
    ...
    try:
        ...
        if due_date is not None:
            # Handle both string and datetime types for due_date
            if isinstance(due_date, str):
                # Parse the ISO 8601 date string with proper timezone handling
                try:
                    # Handle different ISO 8601 formats that may come from the frontend
                    date_str = due_date.strip()

                    # If the date string is empty, set to None
                    if not date_str:
                        ...
                        # Handle the 'T' separator for datetime format
                        if 'T' in date_str:
                            # Replace 'T' with ' ' for fromisoformat compatibility
                    ...
                    raise HTTPException(status_code=400, detail=f"Invalid due_date format. Use ISO 8601 format (e.g., 2023-12-31T18:00:00 or 2023-12-31 18:00:00). Error: {str(e)}")
            elif isinstance(due_date, datetime):
                # If it's already a datetime object (from Pydantic model), use it directly
                todo_data["due_date"] = due_date
            else:
                # If it's neither a string nor datetime, set to None
                todo_data["due_date"] = None
        if recurrence_type is not None:
            todo_data["recurrence_type"] = recurrence_type
        if reminder_time is not None:
            # Ensure reminder_time is converted to integer
            if isinstance(reminder_time, str):
                if reminder_time == "":
                    todo_data["reminder_time"] = 10  # Default value
                else:
                    try:
                        todo_data["reminder_time"] = int(reminder_time)
                    except ValueError:
                        raise HTTPException(status_code=400, detail="Invalid reminder_time format. Must be a number.")
            else:
                # It's already an int
                todo_data["reminder_time"] = int(reminder_time)
...
async def update_todo(
    ...
    try:
        ...
        if reminder_time is not None:
            if isinstance(reminder_time, str):
                if reminder_time == "":
                    # Set to default if empty string is passed
                    todo.reminder_time = 10
                else:
                    try:
                        todo.reminder_time = int(reminder_time)
                    except ValueError:
                        raise HTTPException(status_code=400, detail="Invalid reminder_time format. Must be a number.")
            else:
                # It's already an int
        ...
        if not was_completed and will_be_completed and todo.recurrence_type != RecurrenceEnum.NONE:
            ...
            new_todo_data = {
                ...
                "recurrence_type": todo.recurrence_type,
                "reminder_time": int(todo.reminder_time) if todo.reminder_time is not None else 10,
            }
